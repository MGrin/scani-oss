...
export async function getQueueDepths(): Promise<Result<QueueDepth>> {
  return tryCatch(async () => {
    const [waiting, active, delayed, failed, completed] = await Promise.all([
      redisCmd('LLEN', `bull:${QUEUE}:wait`),
      redisCmd('LLEN', `bull:${QUEUE}:active`),
      redisCmd('ZCARD', `bull:${QUEUE}:delayed`),
      redisCmd('ZCARD', `bull:${QUEUE}:failed`),
      redisCmd('ZCARD', `bull:${QUEUE}:completed`),
    ]);
    return {
      queue: QUEUE,
