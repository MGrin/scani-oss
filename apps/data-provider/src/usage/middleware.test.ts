...
describe('buildUsageMiddleware', () => {
  ...
  it('annotation.provider overrides the inferred provider (e.g. openai vs deepseek)', async () => {
    const sink = new CapturingSink();
    const mw = buildUsageMiddleware({ sink });
    const ctx = makeCtx();
    await mw({
      ...
      type: 'mutation',
      next: async () => {
        ctx.usage.annotate({ provider: 'deepseek', tokensIn: 1024, tokensOut: 256 });
        return { ok: true };
      },
    });
    expect(firstEvent(sink).provider).toBe('deepseek');
