...
test.describe('sessions: list + revoke', () => {
  ...
  test('user can list two sessions and revoke one from another context', async ({
    ...
    const signInB = await pageB.request.post('http://localhost:3011/api/auth/sign-in/email-otp', {
      data: { email, otp },
      headers: { 'content-type': 'application/json', origin: 'http://localhost:5173' },
    });
    ...
    const listRes = await pageA.request.get(
      'http://localhost:3011/trpc/sessions.list?input=%7B%7D'
    );
    ...
    // Revoke B from A
    const revokeRes = await pageA.request.post('http://localhost:3011/trpc/sessions.revoke', {
    ...
    const sessionB = await pageB.request.get(
      'http://localhost:3011/api/auth/get-session?disableCookieCache=true'
    );
