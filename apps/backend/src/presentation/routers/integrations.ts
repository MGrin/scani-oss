...
export const integrationsRouter = router({
  ...
  /**
   * Validate and store Kraken API credentials
   */
  kraken: router({
    validateKeys: protectedProcedure
      .input(
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        let isValid: boolean;
        try {
          isValid = await validateKrakenCredentials(apiKey, apiSecret);
          if (!isValid) {
            throw new TRPCError({
              code: 'BAD_REQUEST',
              message: 'Invalid Kraken API Key or Secret',
            });
          }
        } catch (error) {
          if (error instanceof TRPCError) {
          ...
          throw new TRPCError({
            code: 'BAD_REQUEST',
            message: 'Failed to validate Kraken credentials',
            cause: error,
          });
        }
        ...
        try {
          ...
          // Look up Kraken institution from database
          const krakenInstitution = await db
            .select()
            .from(schema.institutions)
            .where(eq(schema.institutions.name, 'Kraken'))
            .limit(1);

          if (krakenInstitution.length === 0) {
            throw new TRPCError({
              code: 'NOT_FOUND',
              message: 'Kraken institution not found in database. Please run migrations.',
            });
          }

          const krakenInstitutionId = krakenInstitution[0]!.id;
          ...
          // Automatically import accounts and holdings
          const importKrakenAccountsUseCase = Container.get(ImportKrakenAccountsUseCase);
          const importResult = await importKrakenAccountsUseCase.execute({
            userId,
          ...
          return {
            success: true,
            message: 'Kraken credentials validated and stored',
            accounts: importResult.accounts,
            holdings: importResult.holdings,
            accountsCreated: importResult.accountsCreated,
            tokensImported: importResult.tokensImported,
