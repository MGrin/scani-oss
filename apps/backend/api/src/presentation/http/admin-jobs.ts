 ...
 *
 * A trusted operator tool proxies retry/remove requests here so that
 * BullMQ's own state machine — `job.retry()` / `job.remove()` — stays
 * authoritative. We deliberately do NOT expose raw Redis writes from
 * that tool, because BullMQ uses Lua scripts to
