  ...
  app
    .get(
      ...
      async ({ request, body, set }: any) => {
        const actor = await authenticate(request, 'GET', set, body);
    ...
    .put(
      ...
      // biome-ignore lint/suspicious/noExplicitAny: Elysia handler ctx types are dynamic
      async ({ request, body, set }: any) => {
        const actor = await authenticate(request, 'PUT', set, body);
        if (!actor) return authFailureBody(set.status);
        let parsed: unknown;
        try {
          parsed = JSON.parse(verifiedRawBody(body));
        } catch {
          set.status = 400;
    ...
    .post(
      ...
      async ({ request, body, set }: any) => {
        ...
        try {
          parsed = JSON.parse(verifiedRawBody(body)) as typeof parsed;
        } catch {
          set.status = 400;
          return { error: 'body must be JSON' };
        }
