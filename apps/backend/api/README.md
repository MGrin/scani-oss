...
Compiled to a single binary via `bun build --compile` (see `package.json` `build` script + `Dockerfile`). Runtime image is `debian:bookworm-slim` + `/app/server` + curl for the healthcheck. Deploys to Fly as `scani-backend`; configuration in `fly.toml`. Migrations run from CI before the rolling deploy starts.
