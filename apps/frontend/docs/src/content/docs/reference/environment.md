...
| `ROBOTS_POLICY` | app (frontend-app nginx) | Whether a search engine may index this origin. `noindex` (default) keeps the SPA out of search results — `robots.txt`, the `<meta name="robots">` and the `X-Robots-Tag` header all say so, and a crawler takes the most restrictive of the three. `index-root` flips all three together and offers the **root document only**: every other path returns the same SPA shell, so allowing the whole origin offers a crawler an unbounded space of duplicates rather than a site, and blocking `/assets/` would index a page that has not rendered yet. Any other value refuses to start rather than guessing which direction you meant. |
