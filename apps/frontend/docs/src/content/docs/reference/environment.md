...
| `CLOUD_QUOTA_HOURLY_DEFAULT` | app (data-provider) | Requests allowed per API key per rolling hour. **`0` and unset both mean DISABLED** — every key gets an unbounded budget on *your* provider accounts. Applied uniformly to all keys; `cloud_api_keys.quota_monthly_requests` is not read on the request path. |
| `GLOBAL_HOURLY_USD_CAP` | app (data-provider) | Cumulative upstream USD per hour across all tenants. Trips a circuit breaker; further requests get 503 until the next hour-bucket. Decimals allowed. **`0` and unset both mean DISABLED.** |
