...
| Salt Edge | `SALTEDGE_APP_ID`, `SALTEDGE_SECRET`, optional `SALTEDGE_PRIVATE_KEY` | balance, transaction | Bank accounts through Salt Edge's hosted connect widget. The user links a bank on Salt Edge's page; balances and transactions import on the worker and refresh hourly. Listed in the app only when the keys are set. |
...
| Salt Edge | Built, not yet live: it needs Scani's own Salt Edge account keys, and is hidden until they are set. |
