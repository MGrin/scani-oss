...
| `mobile` | Lean, typed read endpoints for the mobile apps (`accounts`, `holdings`) — wraps the domain services with `.output()` schemas; exposed via the OpenAPI mobile contract. |
