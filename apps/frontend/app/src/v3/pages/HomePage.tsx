...
export function HomePage() {
  ...
  //
  // **Why a file leads.** #1069 argued it from SC-450's funnel — 4 of 15
  // accounts connected an integration against 7 of 15 who got a holding in,
  // read as a preference for non-credential paths. The per-job rows do not
  // support that reading and it is corrected here. Of the 7 first attempts on
  // production, 3 were `exchange-import`, 2 manual entry, 1 screenshot, 1
  // wallet and 0 file import; the 4-vs-7 gap is manual entry, not file import,
  // and connecting an exchange is in fact the single most-picked first route.
  //
  // What the rows do say is stronger. Of 15 accounts, 8 never enqueued a
  // capture job of any kind — every one of them with one session or none —
  // and all 7 who did ended up holding a position. Nobody has ever started an
  // import and failed. So the route is not chosen on a conversion rate,
  // because every route converts; it is chosen on what it asks of someone with
  // one session of patience, and `/import` asks for a file and no credential.
  //
