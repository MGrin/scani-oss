...
interface TokenSearchInputProps {
  ...
  label?: string;
  /**
   * Fires when the user picks a token. `details` carries `symbol` + `name`
   * separately so callers that need them (e.g. inline-swap UIs that
   * have to re-render the picked token's symbol after selection) don't
   * have to parse the joined `label` string.
   */
  onSelect: (id: string, label: string, details?: TokenSelectionDetails) => void;
  onClear: () => void;
  disabled?: boolean;
  placeholder?: string;
}
