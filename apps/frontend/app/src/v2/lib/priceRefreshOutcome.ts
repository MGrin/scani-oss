 ...
 *
 * A pure function rather than three copies of the branch, because the sentence
 * is the whole fix and both shells owe the same one.
 */
...
/**
 * @param symbol upper-cased ticker, or null where the caller has no one
 *   holding in hand (v2's detail page speaks about "the price").
 *
...
export function describePriceRefresh(
  ...
  const subject = symbol ? `${symbol} price` : 'The price';
  ...
  if (report && 'price' in report && report.price == null) {
    return {
      kind: 'no-price',
      message: `No provider had a price for ${symbol ?? 'this holding'}. The figure on screen is unchanged.`,
    };
  }
  ...
  if (report?.fetched === false) {
    ...
    const age = report.timestamp ? formatRelative(report.timestamp) : null;
    return {
      kind: 'already-current',
      message: age
        ? `No new price for ${symbol ?? 'this holding'} — showing the one from ${age}`
        : `No new price for ${symbol ?? 'this holding'} — the stored one is unchanged`,
    };
  }

  return { kind: 'updated', message: `${subject} updated` };
}
