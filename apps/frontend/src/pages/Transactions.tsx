...
export function Transactions() {
  ...
  const hasActiveFiltersComputed =
    hasActiveFilters || Boolean(filterValues.search);
  ...
  const effectiveFilterByHolding =
    filterValues.holding !== "all" ? filterValues.holding : null;
  ...
  const holdingFilteredTransactions = effectiveFilterByHolding
    ? baseTransactions.filter((transaction) =>
        holdingFilterIds.includes(transaction.holdingId)
      )
    : baseTransactions;
  ...
  const pageTitle = effectiveFilterByHolding
    ? (() => {
        ...
        return `${
          tokenId ? tokensMap[tokenId]?.name || "Token" : "Token"
        } Transactions`;
      })()
    : "Transactions";
  ...
  return (
    <div className="space-y-4">
      ...
      <PageAggregation
        ...
        totalBalance={
          effectiveFilterByHolding ? totalValue : allTransactionsValue
        }
