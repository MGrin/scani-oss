...
export const validatePositiveNumber = (
  ...
  z.coerce
    ...
    .max(
      maxValue,
      `${fieldName} is too large (maximum: ${maxValue.toLocaleString()})`
    );
...
export const validateDateTimeDefault = () =>
  z
    .date({ invalid_type_error: "Please select a valid date and time" })
    .default(() => new Date());
  ...
  .object({
    ...
    amount: z.coerce
      ...
      .refine(
        ...
        "Amount is too small (minimum: 0.01)"
      )
      .refine(
        (val) => Math.abs(val) <= 1_000_000_000,
        "Amount is too large (maximum: 1 billion)"
      ),
...
export const validateNonEmptyString = (
  ...
  z
    ...
    .refine((str) => str.length > 0, {
      message:
        customMessage || `${fieldName} cannot be empty or contain only spaces`,
    });
