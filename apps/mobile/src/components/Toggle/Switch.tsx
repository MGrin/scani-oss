...
function SwitchInput(props: SwitchInputProps) {
  ...
  return (
    <View style={[$inputOuter, { backgroundColor: offBackgroundColor }, $outerStyleOverride]}>
      ...
      <SwitchAccessibilityLabel {...props} labelType="on" />
      <SwitchAccessibilityLabel {...props} labelType="off" />
...
/**
 * @param {ToggleInputProps & { labelType: "on" | "off" }} props - The props for the `SwitchAccessibilityLabel` component.
 * @returns {JSX.Element} The rendered `SwitchAccessibilityLabel` component.
 */
function SwitchAccessibilityLabel(props: SwitchInputProps & { labelType: 'on' | 'off' }) {
  const { on, disabled, status, accessibilityMode, labelType, innerStyle, detailStyle } = props;
  ...
  const shouldLabelBeVisible = (on && labelType === 'on') || (!on && labelType === 'off');
  ...
  return (
    <View style={$switchAccessibilityStyle}>
      {accessibilityMode === 'text' && shouldLabelBeVisible && (
        <View
          style={[
            labelType === 'on' && $switchAccessibilityLine,
            labelType === 'on' && { backgroundColor: color },
            labelType === 'off' && $switchAccessibilityCircle,
            labelType === 'off' && { borderColor: color },
          ]}
        />
      )}
      ...
      {accessibilityMode === 'icon' && shouldLabelBeVisible && (
        <Image
          ...
          source={labelType === 'off' ? iconRegistry.hidden : iconRegistry.view}
        />
      )}
    </View>
  );
