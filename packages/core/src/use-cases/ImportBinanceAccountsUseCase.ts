 ...
 *
 * Handles importing Binance accounts after API key validation:
 * - Detects available account types (SPOT, MARGIN, FUTURES)
 * - Creates accounts in the database for each available type
 * - Fetches and creates holdings for each account
 * - Stores integration credentials
 *
 * This use case is called after the user validates their Binance API keys
 */
...
const logger = createComponentLogger('use-case:import-binance-accounts');

export interface ImportBinanceAccountsInput {
  /** User ID */
  userId: string;
  /** Institution ID (usually 'binance') */
  institutionId: string;
}

export interface ImportBinanceAccountsResult {
  /** Created accounts */
  accounts: Array<{
    id: string;
    name: string;
...
/**
 * Import Binance Accounts Use Case
 */
@Service()
export class ImportBinanceAccountsUseCase {
  ...
  async execute(input: ImportBinanceAccountsInput): Promise<ImportBinanceAccountsResult> {
    logger.info(
      ...
      'Starting Binance accounts import'
    );

    const result: ImportBinanceAccountsResult = {
      accounts: [],
      holdings: [],
      accountsCreated: 0,
      tokensImported: 0,
    ...
    try {
      ...
      // Fetch accounts from Binance
      logger.debug('Fetching accounts from Binance');
      ...
      if (accountsResult.accounts.length === 0) {
        logger.warn('No accounts returned from Binance');
        return result;
      }
      ...
      for (const accountInfo of accountsResult.accounts) {
        try {
          ...
          if (existing) {
            ...
            logger.info({ accountId, accountType: accountInfo.accountType }, 'Created new account');
          }

          // Fetch holdings for this account
          logger.debug({ accountId }, 'Fetching holdings');
      ...
      logger.info(
        ...
        'Binance accounts import completed'
      );
      ...
      logger.error(
        { error: error instanceof Error ? error.message : String(error) },
        'Failed to import Binance accounts'
      );
      throw error;
    }
  }
