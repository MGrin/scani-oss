...
export class ImportBinanceAccountsUseCase {
  ...
  async execute(input: ImportBinanceAccountsInput): Promise<ImportBinanceAccountsResult> {
    ...
    try {
      ...
      // Fetch accounts from Binance API (external call)
      logger.debug('Fetching accounts from Binance API');
      ...
      for (const accountInfo of accountsResult.accounts) {
        logger.debug(
          ...
          'Fetching holdings from Binance API'
        );
      ...
      await withTransaction(
        ...
        { name: 'importBinanceAccounts', timeout: 60000 }
      );
