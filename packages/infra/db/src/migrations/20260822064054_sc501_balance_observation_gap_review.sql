...
-- "never asked", which is true of all 72,998 of them.
...
-- touches the heap.
--
-- Measured on production 2026-08-22 WITHOUT this index, for the heaviest
-- account (69,546 observations): 900ms, of which 620ms was a bitmap heap scan
-- feeding a sort. That the index removes the sort and the heap fetch is the
-- planner's documented behaviour for a covering btree matching the window's
-- PARTITION BY / ORDER BY — it has NOT been measured at that row count, which
-- is why the number above is dated and attributed and this sentence is not a
-- claim about a speed.
