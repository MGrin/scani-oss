...
-- window. One production cash holding carries eight rows dated months earlier
-- than the range its statement covers — quarterly fees, reported inside the
-- window and dated to the quarter they accrued in. Deriving the boundary from
-- `first_tx_at` there would announce a window months wider than the one that
-- was actually fetched, and print an explanation over the genuinely unfetched
-- months between. A false explanation over a real gap is strictly worse than
-- the honest "unexplained" it replaces, which is why this is stated by the
-- source rather than inferred from what the source happened to send.
--
