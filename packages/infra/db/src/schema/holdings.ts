...
export const holdingTransactions = pgTable(
  'holding_transactions',
  {
    ...
    // that reading was ~99.8% accurate the day it was written — every write
    // path set both columns together, and 560 of 561 answered rows had no
    // timestamp, so *stamped* and *a person answered* picked out the same rows.
    //
    // Rows then acquired timestamps without sources and it inverted, silently.
    // Measured on production 2026-08-26, over the 79 `left_control` rows
    // feeding observed burn: **79.6% of the value decoded as `user`, and 23.7%
    // carried a user stamp.** The prescription outlived the data shape that
