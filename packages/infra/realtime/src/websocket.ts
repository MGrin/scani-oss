...
const REQUIRED_SERVICE_NAME = 'api';
...
export class WebSocketRealtimeUpdatesService extends RealtimeUpdatesService {
  ...
  // old guard fired ~50 spurious Sentry events on boot when SERVICE_NAME
  // was still unset at construction time. The check is now in the
  // api-only entry points (setElysiaApp / initialize / pipeFromRedis);
