...
| `WebSocketRealtimeUpdatesService` | Owns local connection bookkeeping (registerConnection / handleMessage / handleDisconnection / heartbeat) and forwards Redis-piped messages to local WS clients via `setElysiaApp(app)`. **Constructor throws if `SERVICE_NAME !== 'scani-backend'`** — only the api process should hold WS state. |
...
`process.env.SERVICE_NAME !== 'scani-backend'`. Connection state has no
