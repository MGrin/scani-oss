...
type UserJobName =
  ...
  | typeof JOB_NAMES.transactionImport;
...
type UserJobDataMap = {
  ...
  [JOB_NAMES.transactionImport]: TransactionImportJob;
};
...
const DEFAULT_OPTS: Record<UserJobName, JobsOptions> = {
  ...
  [JOB_NAMES.transactionImport]: {
    // Upstream HTTP APIs are flaky; give a few retries with generous
    // backoff since histories can span years and a mid-fetch 429 from
    // Etherscan/Kraken is common. The processor itself is idempotent
    // per (account, source, external_id) so retry is safe.
    attempts: 4,
...
export function computeJobId<Name extends UserJobName>(
  ...
  switch (name) {
    ...
    case JOB_NAMES.transactionImport: {
      const d = data as TransactionImportJob;
      // Dedup key: (user, account, source, requestId). A re-ingest with
      // a fresh requestId gets a new id; accidental double-click with
      // the same requestId collapses via BullMQ's native dedup.
      return [name, d.userId, d.accountId, d.source, d.requestId].join(JOB_ID_SEP);
    }
    default: {
