...
| Var | Default | Purpose |
|---|---|---|
| `SERVICE_NAME` | `scani` | Goes into every log line's `service` field. Each app's `fly.toml` / `docker-compose.yml` sets this so a shared log stream can tell `scani-backend` / `scani-worker` / `scani-data-provider` rows apart. |
