...
describe('BinanceProvider', () => {
  ...
  test('fetchBalances merges spot + margin and skips zero-balance rows', async () => {
    const p = new BinanceProvider(passthroughLimiter());
    ...
    try {
      ...
      expect(calls).toBe(2); // spot + margin
      ...
      expect(byAsset.USDT).toBe('50'); // 0 spot dropped, margin 50 kept
    } finally {
