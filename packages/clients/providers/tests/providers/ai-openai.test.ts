...
describe('OpenAIProvider', () => {
  ...
  test('parseScreenshot posts to /chat/completions with vision model', async () => {
    ...
    const capturedBodies: Array<{ model: string; messages: unknown[] }> = [];
    ...
    try {
      ...
      expect(capturedBodies[0]?.model).toBe('gpt-5.6-luna');
    } finally {
  ...
  test('parseScreenshot sends a PDF as a file part, never as an image part', async () => {
    ...
    try {
      ...
      const parts = capturedBodies[0]?.messages[1]?.content as Array<Record<string, unknown>>;
  ...
  test('parseScreenshot systemPrompt replaces the default holdings prompt', async () => {
    ...
    try {
      ...
      const system = capturedBodies[0]?.messages[0];
