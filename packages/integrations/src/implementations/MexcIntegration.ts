/**
 * MexcIntegration - API Key Integration for MEXC Exchange
 *
 * Handles MEXC trading accounts using API Key + Secret authentication.
 * Very similar to Binance in terms of API structure.
 */

import { ScaniIntegration } from '../base';
import type { MexcApiService } from '../services/MexcApiService';
import type {
  AuthConfig,
  FetchAccountsResult,
  FetchHoldingsResult,
...
export class MexcIntegration extends ScaniIntegration {
  ...
  async fetchAccounts(credentials?: Record<string, unknown>): Promise<FetchAccountsResult> {
    try {
      ...
      // MEXC has a single unified spot trading account
      const accountUid = 'mexc-api-account';
      const accounts = [
        {
          externalId: `SPOT_${accountUid}`,
          name: 'MEXC Spot Trading',
          accountType: 'SPOT',
          description: 'MEXC Spot Trading Account',
          metadata: {
            uid: accountUid,
            accountType: 'SPOT',
            provider: 'mexc',
  ...
  async fetchHoldings(
    ...
    try {
      ...
      try {
        balances = await this.mexcService.getBalances(apiKey, apiSecret);
      } catch (error) {
        return {
          holdings: [],
          total: 0,
