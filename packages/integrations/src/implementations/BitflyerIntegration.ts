/**
 * BitflyerIntegration - API Key Integration for bitFlyer (Japan)
 *
 * Docs: https://lightning.bitflyer.com/docs?lang=en
 */

import { ScaniIntegration } from '../base';
import type { BitflyerApiService } from '../services/BitflyerApiService';
import type {
  AuthConfig,
  FetchAccountsResult,
  FetchHoldingsResult,
...
export class BitflyerIntegration extends ScaniIntegration {
  private readonly apiService: BitflyerApiService;

  constructor(
    institutionId: string,
    authConfig: AuthConfig,
    apiService: BitflyerApiService,
  ...
  async fetchAccounts(credentials?: Record<string, unknown>): Promise<FetchAccountsResult> {
    try {
      ...
      return {
        accounts: [
          {
            externalId: 'TRADING_bitflyer-api-account',
            name: 'bitFlyer Trading Account',
            accountType: 'TRADING',
            description: 'bitFlyer Lightning trading account',
            metadata: { provider: 'bitflyer', accountType: 'TRADING' },
            isActive: true,
          },
        ],
        total: 1,
  ...
  async fetchHoldings(
    ...
    try {
      ...
      const holdings: IntegrationHolding[] = rows.map((row) => {
        const symbol = row.currency_code.toUpperCase();
        return {
          symbol,
          name: symbol,
          ...
          externalTokenId: row.currency_code,
          metadata: {
            accountType: 'TRADING',
