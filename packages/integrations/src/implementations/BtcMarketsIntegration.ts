/**
 * BtcMarketsIntegration - API Key Integration for BTC Markets (Australia)
 *
 * Docs: https://docs.btcmarkets.net/
 */

import { ScaniIntegration } from '../base';
import type { BtcMarketsApiService } from '../services/BtcMarketsApiService';
import type {
  AuthConfig,
  FetchAccountsResult,
  FetchHoldingsResult,
...
export class BtcMarketsIntegration extends ScaniIntegration {
  private readonly apiService: BtcMarketsApiService;

  constructor(
    institutionId: string,
    authConfig: AuthConfig,
    apiService: BtcMarketsApiService,
  ...
  async fetchAccounts(credentials?: Record<string, unknown>): Promise<FetchAccountsResult> {
    try {
      ...
      const accounts = [
        {
          externalId: 'TRADING_btc-markets-api-account',
          name: 'BTC Markets Trading Account',
          accountType: 'TRADING',
          description: 'BTC Markets Trading Account',
          metadata: { provider: 'btc_markets', accountType: 'TRADING' },
          isActive: true,
        },
      ];
  ...
  async fetchHoldings(
    ...
    try {
      ...
      const holdings: IntegrationHolding[] = rows.map((row) => {
        const symbol = row.assetName.toUpperCase();
        return {
          symbol,
          name: symbol,
          balance: row.balance,
