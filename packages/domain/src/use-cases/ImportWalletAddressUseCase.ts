...
import { WarmTokenPricesForImportUseCase } from './WarmTokenPricesForImportUseCase';
...
export class ImportWalletAddressUseCase {
  // Constructor injection rather than `private readonly foo = Container.get(Foo)`
  // in field initializers: the dependency graph is visible at a glance,
  // and tests can instantiate this class directly with stub services
  // (no `Container.set` required).
  constructor(
    ...
    private readonly warmTokenPrices: WarmTokenPricesForImportUseCase = Container.get(
      WarmTokenPricesForImportUseCase
    )
  ) {}
  ...
  private async executeWithIntegrations(
    ...
    if (input.detectedInstitutionIds && input.detectedInstitutionIds.length > 0) {
      ...
      logger.debug(
        ...
        'Detecting wallet chains'
      );

      // Single code path through IntegrationManager — it owns the raw
      // per-chain clients AND the institution-mapping lookup. See
      // IntegrationManager.detectWalletInstitutions.
      detectedInstitutionIds = await this.integrationManager.detectWalletInstitutions(
        input.address
      );
    ...
    if (!input.displayName) {
      try {
        const ensName = await this.integrationManager.resolveEnsName(input.address);
        if (ensName) {
    ...
    const prices = await this.warmTokenPrices.execute({
      userId,
      tokenIds: result.holdings.map((h) => h.tokenId),
    });
