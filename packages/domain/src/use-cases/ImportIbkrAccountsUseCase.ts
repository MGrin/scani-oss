...
export class ImportIbkrAccountsUseCase {
  ...
  async execute(input: ImportIbkrAccountsInput): Promise<ImportIbkrAccountsResult> {
    ...
    try {
      ...
      // If the integration reports errors AND produced no accounts, the
      // import has failed entirely — surface the real reason to the user
      // by failing the job. Silently returning an empty result lands them
      // on an empty holdings page with no clue what went wrong.
      if (accountsResult.accounts.length === 0) {
        const reason =
          accountsResult.errors && accountsResult.errors.length > 0
            ? accountsResult.errors.join('; ')
            : 'IBKR returned no accounts';
        logger.error({ errors: accountsResult.errors }, 'IBKR accounts import failed');
        throw new Error(`IBKR import failed: ${reason}`);
      }

      if (accountsResult.errors && accountsResult.errors.length > 0) {
        logger.warn({ errors: accountsResult.errors }, 'Partial errors fetching IBKR accounts');
        // Non-empty accounts + non-empty errors — keep result.errors so
        // the UI can show partial failures. This carries through below.
        result.errors.push(
      ...
      // If every account's holdings fetch failed (no holdings anywhere
      // AND errors were recorded), fail the job — `fetchAccounts` returns
      // a stub PORTFOLIO account unconditionally, so a "1 account / 0
      // holdings / errors" outcome previously reported as success with
      // silent errors inside. That's the "job succeeded but had errors"
      // bug the user hit. Explicitly throw so the processor marks the
      // job failed and the user sees the real cause.
      ...
      if (totalHoldings === 0 && hadErrors) {
        const reason = result.errors.map((e) => e.error).join('; ');
        logger.error({ errors: result.errors }, 'IBKR import produced no holdings');
        throw new Error(`IBKR import failed: ${reason}`);
      }
