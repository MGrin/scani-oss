import type { ParsedHolding, ParsedPortfolio } from '@scani/ai-providers';
...
/**
 * AI-driven screenshot → portfolio extraction.
 *
 * Split out of `AIService` so the screenshot pipeline (image → LLM →
 * validate-each-holding → confidence filter) is self-contained. Shares
 * the AI provider manager with CSV-column detection via
 * `AIProviderService`.
 */
@Service()
export class ScreenshotParsingService extends BaseService {
  ...
  async parseScreenshot(
    ...
    try {
      ...
      const aiResponse = await manager.parseScreenshot(imageBase64, {
  ...
  async parseDocumentText(
    ...
    try {
      ...
      const aiResponse = await manager.parseDocumentText(text, {
        provider: options?.provider,
        accountType: options?.accountType,
        expectedCurrency: options?.expectedCurrency,
        context: options?.context,
