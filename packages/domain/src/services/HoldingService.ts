...
export class HoldingService extends BaseService {
  // Each dependency below has exactly one call site (grep the file).
  // Class-field deps: the two collaborators used across the mutation
  // methods (create/update/delete all need these). The summary-only
  // collaborators (`GroupRepository`, `HoldingApyConfigRepository`,
  // `PortfolioValuationService`) are resolved lazily from the container
  // inside the one method that uses them, so the "mutations inherit a
  // PortfolioValuationService dep they never use" smell is gone.
  ...
  async getHoldingsByAccountIdWithDetails(
    ...
    // Lazy-resolve summary-only deps so mutation methods don't inherit them.
    ...
    const holdingApyConfigRepository = Container.get(HoldingApyConfigRepository);
    ...
    const [groupsMap, apyConfigsMap] = await Promise.all([
      groupRepository.findGroupsForHoldings(
        holdingsWithFullDetails.map(({ holding, account }) => ({
          id: holding.id,
          accountId: account.id,
        }))
      ),
      holdingApyConfigRepository.findByHoldingIds(holdingIds),
    ]);
  ...
  /**
   * Find holding by ID — matches `HoldingRepository`'s two explicit
   * methods: `findByIdVisible` applies the dashboard's hidden-filter;
   * the unfiltered variant is the base `findById`.
   */
