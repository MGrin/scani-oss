...
export class OpenAIProvider extends AIProvider {
  ...
  /**
   * Text-only variant of `parseScreenshot`. Used for PDFs where the
   * Chat Completions vision endpoint rejects `application/pdf`; we
   * extract text upstream with pdf-parse and feed it here. Prompt and
   * JSON shape match `parseScreenshot` exactly so the downstream
   * validation/enrichment pipeline doesn't care which entry point was
   * used.
   */
  override async parseDocumentText(
    text: string,
    options?: {
      accountType?: string;
      expectedCurrency?: string;
...
Below is the plain-text content of a financial statement PDF. Use it the same way you would a screenshot.
