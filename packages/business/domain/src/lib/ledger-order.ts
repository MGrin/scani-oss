 ...
 * different plan. One SOL holding read 63.05 / 51.39 / 38.96 across three
 * database states whose ledgers were md5-identical. 544 of 2084 production
 * rows (26%) share an exact `occurred_at` with a sibling in the same holding.
 *
 ...
 *    text order *is* ascending execution order, and it discriminates all 25
 *    of the production tie groups on that source. Where the provider issues
 ...
 *    `walkComponent` flattens several holdings into one sequence. 23
 ...
 * it ties for **every** production tie group (113 of 113 on `solana`). It
