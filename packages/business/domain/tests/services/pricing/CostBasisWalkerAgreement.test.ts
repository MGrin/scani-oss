 ...
 * history` failed on 791 of 2,048 histories, and the round-trip fixture below
 * reported realized -1,900 through `walkLots` against 500 through
 * `walkComponent` — 2,700 of cost invented by re-minting a returning lot at the
 * inflow leg's price.
 */
...
describe('a self-spanning transfer group is a no-op (SC-344)', () => {
  ...
  test('the round trip neither destroys cost basis nor realizes a gain', async () => {
    ...
    // lot there is what produced the 0.53 discrepancy in production, and at
    // this fixture's scale it would be visible as 2,700 of invented cost.
