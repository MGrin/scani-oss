...
describe('SC-661 — one division, shared by both surfaces', () => {
   ...
   * window, and mgrin's book nets +$10,***REMOVED*** a month, so neither ever did —
  ...
  test('a purchase always costs a statable number of months', () => {
    ...
    expect(verdict?.remaining.toString()).toBe('82***REMOVED***');
  });
