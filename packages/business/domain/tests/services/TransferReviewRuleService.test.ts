...
describe('TransferReviewRuleService — payment descriptions', () => {
  ...
  const NIKITA = [
    'Pay 2500.00 USD to Example Recipient (Dividends)',
    'Pay 300.00 USD to Example Recipient (Dividends)',
    'Pay 3000.00 USD to Example Recipient (Dividends)',
    'Pay 4000.00 USD to Example Recipient (Dividends)',
    'Pay 4500 USD to Example Recipient (Dividends)',
    'Pay 500.00 USD to Example Recipient (Dividends)',
    'Pay 500.00 USD to Example Recipient (Dividends)',
  ];
  const AMARA = [
  ...
  ***REMOVED***
  ...
  test("the source data's own typo keys separately, and that is correct", async () => {
    const f = fixture!;
    await payment(f, 'p-typo-1', 'Pay 2500.00 USD to Example Recipient (Dividends)');
    const typo = await payment(f, 'p-typo-2', 'Pay 1500.00 USD to Example Recipient (Dividents)');
  ...
  test('two recipients sharing a prefix stay apart', async () => {
    const f = fixture!;
    const first = await payment(f, 'p-prefix-1', 'Pay 100.00 USD to Example Recipient');
    await payment(f, 'p-prefix-2', 'Pay 100.00 USD to Example Recipienta');
  ...
  test('re-keying an already-normalized key is a no-op', async () => {
    ...
    const [row] = await db.execute<{ once: string | null; twice: string | null }>(
      sql`select
            transfer_counterparty_key('Pay 500.00 USD to Example Recipient (Dividends)') as once,
            transfer_counterparty_key(
              transfer_counterparty_key('Pay 500.00 USD to Example Recipient (Dividends)')
            ) as twice`
    );
