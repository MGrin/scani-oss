/**
 * Capture landing-page product screenshots in light + dark, at a
 * desktop viewport, against a real running app. Authenticates via the
 * screenshot-bot Better-Auth plugin (see
 * `apps/backend/api/src/auth/screenshot-bot-plugin.ts`).
 *
 * Outputs `apps/frontend/landing/public/screenshots/<slug>-<theme>-desktop.png`
 * — consumed by `apps/frontend/landing/src/components/sections/ProductShowcase.tsx`,
 * which picks the variant matching the visitor's system theme.
 *
 * Required env:
    ...
    for (const theme of THEMES) {
      ...
      for (const shot of SHOTS) {
        ...
        try {
          ...
          // `-desktop` suffix kept so the committed PNG filenames stay
          // stable; the landing only ships a desktop variant now.
          const out = resolve(outDir, `${shot.slug}-${theme}-desktop.png`);
          await page.screenshot({ path: out, fullPage: false });
          console.log(`  [${shot.slug}-${theme}] → ${out}`);
        } finally {
          await page.close();
        }
      }
