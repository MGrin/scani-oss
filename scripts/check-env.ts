...
if (!hasBackendEnv) {
  ...
  console.log(
    `${YELLOW}   Copy apps/backend/.env.example to apps/backend/.env.local${RESET}`
  );
  allValid = false;
} else {
...
if (!hasFrontendEnv) {
  ...
  console.log(
    `${YELLOW}   Copy apps/frontend/.env.example to apps/frontend/.env.local${RESET}`
  );
  allValid = false;
} else {
...
console.log(
  `1. Create a Supabase project at ${BLUE}https://supabase.com${RESET}`
);
console.log(`2. Enable Email/Password authentication in Supabase Dashboard`);
console.log(
  `3. Copy your project URL and keys from Supabase Dashboard > Settings > API`
);
console.log(
  `4. Copy the example environment files and add your Supabase credentials:`
);

if (!hasBackendEnv) {
  console.log(
    `   ${YELLOW}cp apps/backend/.env.example apps/backend/.env.local${RESET}`
  );
}
if (!hasFrontendEnv) {
  console.log(
    `   ${YELLOW}cp apps/frontend/.env.example apps/frontend/.env.local${RESET}`
  );
}
...
if (allValid) {
  ...
  console.log(
    `${GREEN}You can now run 'bun dev' to start the application.${RESET}`
  );
} else {
