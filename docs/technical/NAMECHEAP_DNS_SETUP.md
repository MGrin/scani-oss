...
Assuming you own a domain (e.g., `scani.com`), we'll configure:
- **Frontend**: `app.scani.com` → Scani app
- **Backend**: `api.scani.com` → API endpoints
- **Root** (optional): `scani.com` → Redirect to `app.scani.com`
...
3. **Add Custom Domain**:
   ...
   - Enter: `app.scani.com` (or `www.scani.com` if you prefer)
   - Click **"Save"**
...
3. **Open DNS Management**:
   - Find your domain (e.g., `scani.com`)
...
If you want `scani.com` to redirect to `app.scani.com`:
...
2. **Configure**:
   ...
   - **Value**: Enter `https://app.scani.com`
...
3. Set redirect from `scani.com` → `https://app.scani.com`
...
CNAME Record  app     scani-frontend.onrender.com.       Automatic
CNAME Record  api     scani-backend-217c.onrender.com.   Automatic
URL Redirect  @       https://app.scani.com              Automatic
```
...
2. Check `app.scani.com` → Should point to `scani-frontend.onrender.com`
3. Check `api.scani.com` → Should point to `scani-backend-217c.onrender.com`

**Using Command Line**:
```bash
# Check frontend DNS
nslookup app.scani.com
# Should show: app.scani.com → scani-frontend.onrender.com

# Check backend DNS
nslookup api.scani.com
# Should show: api.scani.com → scani-backend-217c.onrender.com

# Alternative: use dig
dig app.scani.com CNAME
dig api.scani.com CNAME
```
...
3. **Update FRONTEND_URL**:
   ...
   - Change value to: `https://app.scani.com`
   - Click **Save**
...
3. **Update VITE_API_URL**:
   ...
   - Click **Edit**
   - Change value to: `https://api.scani.com`
   - Click **Save**
...
3. **Update Site URL**:
   ...
   - Change to: `https://app.scani.com`
   - Click **Save**

4. **Update Redirect URLs**:
   - Add: `https://app.scani.com/**`
...
1. **Test Health Endpoint**:
   ```bash
   curl https://api.scani.com/health
   ```
   - Should return: `{"status":"ok","timestamp":"..."}`

2. **Open in Browser**:
   - Go to: `https://api.scani.com/health`
...
3. **Check Email**:
   ...
   - Verify link points to: `https://app.scani.com/...`
   - Not `localhost` or `.onrender.com`

4. **Complete Login**:
   - Click the magic link
   - Should redirect to `https://app.scani.com`
...
3. **Verify**:
   - API calls should go to: `https://api.scani.com/trpc/...`
...
- [ ] `app.scani.com` resolves to `scani-frontend.onrender.com` (verify with nslookup)
- [ ] `api.scani.com` resolves to `scani-backend-217c.onrender.com` (verify with nslookup)
...
- [ ] `https://app.scani.com` loads without SSL warnings
- [ ] `https://api.scani.com/health` returns JSON response
- [ ] Backend `FRONTEND_URL` updated to `https://app.scani.com`
- [ ] Frontend `VITE_API_URL` updated to `https://api.scani.com`
- [ ] Both services redeployed after env var updates
- [ ] Supabase Site URL updated to `https://app.scani.com`
- [ ] Supabase Redirect URLs includes `https://app.scani.com/**`
...
```
Site URL: https://app.scani.com
Redirect URLs: https://app.scani.com/**
```
...
1. ✅ `https://app.scani.com` loads your app with HTTPS
2. ✅ `https://api.scani.com/health` returns JSON
3. ✅ Login magic link points to `app.scani.com`
...
**Ready to start?** Just replace `scani.com` with your actual domain throughout this guide!
