...
- **Top-level imports only.** No `await import(...)`, no `require()`. If a
  ...
  dependencies are statically resolvable. The three backend `index.ts` files
  that carried this as debt are clean as of 2026-09-09.
...
## The three paths a `find` will not tell you

Everything else in the tree is where its name says it is; these three are not.

- **Queue names** are `packages/infra/queue/src/core/default-names.ts`. There is no
  `queue-names.ts` and no `enqueue.ts` — this file named both until 2026-09-09.
...
- **Coverage**: `bun test --coverage` (per-package, on-demand). Also run in CI by
  `coverage.yml`, on every push to `main` — it writes the badge gist. PRs are
  excluded on purpose: a fork PR cannot read the secret, and updating the badge
  from a PR would clobber main's number.
...
**Eleven workflows live in `.github/workflows/` and that directory is the source of
truth** — this section describes only the ones whose behaviour is not obvious from
reading them.

`ci.yml` is path-filtered off a `detect-changes` job, and gates on `ci-success`:

- `validate-code` — Biome lint + parallel `tsgo --noEmit` across all workspaces.
- `validate-deps` — `bun run deps:lint`, `bun run deps:unused` and
  `bun run ci:gen:check`, when lockfile/config files **or** source files changed.
  It invokes the package scripts rather than inlining their flags, and so does this
  line as of SC-739: the flags were copied into prose here and drifted at SC-558
  without anything noticing.
- `validate-migrations` — `bun run db:drift:check`, on a full-history checkout.
  Unfiltered and ungated: a migration edited with no `DRIFT_DECLARATIONS` entry
  refuses every deploy, and a base a shallow clone cannot resolve is exit 9 rather
  than a pass.
- `test` — Postgres 16 service container; `bun run db:migrate`, then **`bun run test`**
  — the package script, not an inlined `bun test` line, for the reason "Before
  Pushing" gives.
- `validate-docs`, `e2e`, `prod-guards-integration`, `oss-boundary`, `secret-scan`.

Everything else is its own file: `coverage.yml` (push to `main`, writes the badge
gist), `codeql.yml`, `security-invariants.yml`, `deploy-docs.yml`,
`docker-publish.yml` (**live, not planned** — it builds and publishes the multi-arch
image on a `v*` tag), `release-please.yml` and `release-notes.yml`, `pr-title.yml`
and `pr-body.yml`, `sync-dockerhub-readmes.yml`.
...
**Repeatable jobs**: one file per job in
`packages/business/jobs/src/scheduled-jobs/` — 27 of them as of 2026-09-09 — and each
file carries its own cron string. **That directory is the list.** A hand-copied
enumeration used to sit here and had drifted to naming 12 of them, which is worse
than none: a reader checks the list, does not find a job, and concludes it does not
exist.

**User-initiated jobs**: `packages/business/jobs/src/job-names.ts`, same rule.
